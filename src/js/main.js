...
function render() {
  ...
  fetchEmailsFromDatabase(cursor = 0).then(function(emails) {

    var filteredEmails = getFilteredEmails(emails, getSearchInputs() );
...
function getFilteredEmails(allEmails = [], searchInputs = getSearchInputs()) {
  ...
  // It works as it is. However, there's a more efficient way to implement it
  // Especially when the searchInputs is returned from the search input
  var filteredEmails = _.pickBy(allEmails, function(item) {
    return item.author.indexOf(searchInputs[0]) > -1 || item.subject.indexOf(searchInputs[1]) > -1 || item.body.indexOf(searchInputs[2]) > -1;
  });
...
function fetchEmailsFromDatabase(cursor = 0, callback) {
  ...
  var promise = new Promise(function(resolve, reject) {
    window.setTimeout(function() {
      ...
      const next = Math.min(cursor + _.random(1, 7), last);
